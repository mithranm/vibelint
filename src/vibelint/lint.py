...
class LintRunner:
    ...
    def run(self, paths: List[Path]) -> int:
        ...
        if not self.skip_confirmation and len(python_files) > self.config["large_dir_threshold"]:
        ...
        with Progress(
            ...
            task = progress.add_task(f"Linting {len(python_files)} Python files...", total=len(python_files))
    ...
    def _process_file(self, file_path: Path) -> LintResult:
        ...
        try:
            ...
            # Apply validations
            is_script = re.search(r"if\s+__name__\s*==\s*['\"]__main__['\"]", content) is not None
            ...
            # Validate shebang
            shebang_result = validate_shebang(content, is_script, self.config["allowed_shebangs"])
            if shebang_result.has_issues():
                result.errors.extend(shebang_result.errors)
                result.warnings.extend(shebang_result.warnings)
                if not self.check_only:
                    new_content = fix_shebang(new_content, shebang_result, is_script, self.config["allowed_shebangs"][0])
            ...
            if docstring_result.has_issues():
                ...
                if not self.check_only:
                    new_content = fix_module_docstring(new_content, docstring_result, relative_path)
    ...
    def _print_summary(self):
        ...
        table.add_row("Files with errors", str(self.files_with_errors), style="red" if self.files_with_errors else "green")
        table.add_row("Files with warnings", str(self.files_with_warnings), style="yellow" if self.files_with_warnings else "green")
        ...
        if self.files_with_errors > 0 or self.files_with_warnings > 0:
            ...
            for result in self.results:
                if result.has_issues:
                    status = "[red]ERROR[/red]" if result.errors else "[yellow]WARNING[/yellow]"
