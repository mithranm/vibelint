...
- Primary: Fast inference for quick tasks (docstrings, simple analysis)
...
class LLMRole(Enum):
    ...
    PRIMARY = "primary"           # Fast inference, small context
...
class LLMManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.primary_config = {
            "api_url": llm_config.get("primary_api_url"),
            "model": llm_config.get("primary_model"),
            "temperature": llm_config.get("primary_temperature", 0.1),
            "max_tokens": llm_config.get("primary_max_tokens", 2048)
        }
    ...
    async def process_request(self, request: LLMRequest) -> Dict[str, Any]:
        ...
        try:
            if selected_llm == LLMRole.PRIMARY:
                return await self._call_primary_llm(request)
            else:
            ...
            if self.enable_fallback:
                ...
                # Try the other LLM
                fallback_role = LLMRole.ORCHESTRATOR if selected_llm == LLMRole.PRIMARY else LLMRole.PRIMARY

                try:
                    if fallback_role == LLMRole.PRIMARY:
                        result = await self._call_primary_llm(request)
                    else:
    ...
    async def _call_primary_llm(self, request: LLMRequest) -> Dict[str, Any]:
        """Call the primary (fast) LLM."""
        return await self._make_api_call(self.primary_config, request, LLMRole.PRIMARY)
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            "primary_configured": bool(self.primary_config.get("api_url")),
...
def create_llm_manager(config: Dict[str, Any]) -> Optional[LLMManager]:
    ...
    has_primary = bool(llm_config.get("primary_api_url"))
    has_orchestrator = bool(llm_config.get("orchestrator_api_url"))

    if not has_primary and not has_orchestrator:
