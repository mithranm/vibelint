...
class ProjectCleanupWorkflow:
    ...
    def _find_untracked_important_files(self) -> List[Dict[str, Any]]:
        ...
        if result.returncode == 0:
            ...
            raise RuntimeError(f"git ls-files failed with return code {result.returncode}: {result.stderr}")
    ...
    def commit_cleanup_results(self, results: Dict[str, Any], cleanup_name: str):
        ...
        commit_msg += f"Space saved: {space_saved / (1024*1024):.1f} MB\n"
        commit_msg += f"Actions executed: {len(executed)}\n"
        commit_msg += f"Actions skipped: {len(skipped)}\n\n"

        if executed:
            commit_msg += "Executed cleanup actions:\n"
            for action in executed:
                commit_msg += f"- {action.get('description', str(action))}\n"

        if errors:
            commit_msg += f"\nErrors encountered: {len(errors)}\n"
            for error in errors:
                commit_msg += f"- {error.get('error', str(error))}\n"
