...
class JustificationAnalysisWorkflow:
    ...
    def _analyze_python_file(self, file_path: Path, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "analysis_quality": self._assess_analysis_quality(file_justification, method_justifications)
        }
    ...
    def _analyze_file_justification(self, file_path: Path, tree: ast.AST, content: str) -> FileJustification:
        ...
        primary_purpose = self._infer_primary_purpose(file_path, classes, functions, module_docstring)
        ...
        if self.model:
            purpose_text = self._create_purpose_text(file_path, primary_purpose, secondary_purposes, module_docstring)
            if purpose_text:
                try:
    ...
    def _analyze_methods_in_file(self, file_path: Path, tree: ast.AST, content: str) -> List[MethodJustification]:
    ...
    def _analyze_single_method(self, file_path: Path, node: ast.FunctionDef, content: str) -> Optional[MethodJustification]:
        ...
        purpose_embedding = None
        if self.model:
            purpose_text = self._create_method_purpose_text(node.name, docstring, unique_functionality)
            if purpose_text:
                try:
                    embedding = self.model.encode([purpose_text])[0]
                    purpose_embedding = embedding.tolist()
    ...
    def _infer_primary_purpose(self, file_path: Path, classes: List[ast.ClassDef],
                              functions: List[ast.FunctionDef], docstring: Optional[str]) -> str:
        ...
        if filename.endswith('_test') or filename.startswith('test_'):
            ...
            return f"class_definitions"
        elif len(functions) > len(classes) and functions:
            return f"functional_operations"
        elif docstring:
            return self._extract_purpose_from_docstring(docstring)
        else:
            return f"unclear_purpose"

    def _identify_secondary_purposes(self, classes: List[ast.ClassDef],
                                   functions: List[ast.FunctionDef], content: str) -> List[str]:
        """Identify secondary purposes beyond the primary file purpose."""
        secondary = []

        # Check for helper functions
        helper_funcs = [f for f in functions if f.name.startswith('_') and not f.name.startswith('__')]
        if helper_funcs:
    ...
    def _describe_unique_functionality(self, node: ast.FunctionDef, docstring: Optional[str]) -> str:
    ...
    def _analyze_generic_file_justification(self, file_path: Path) -> FileJustification:
        ...
        if self.model and content_sample:
            purpose_text = self._create_generic_purpose_text(file_path, primary_purpose, content_sample)
            if purpose_text:
                try:
                    embedding = self.model.encode([purpose_text])[0]
                    purpose_embedding = embedding.tolist()
    ...
    def _infer_generic_file_purpose(self, file_path: Path) -> str:
        ...
        # Configuration files
        if filename in ['pyproject.toml', 'setup.py', 'setup.cfg', 'requirements.txt', 'pipfile',
                       'poetry.lock', 'package.json', 'yarn.lock', 'composer.json']:
            return "project_configuration"

        if filename in ['.gitignore', '.gitattributes', '.gitmodules', '.pre-commit-config.yaml',
                       '.github', '.gitlab-ci.yml', 'tox.ini', '.flake8', '.pylintrc']:
        ...
        # Documentation
        if suffix in ['.md', '.rst', '.txt'] and any(doc_word in filename for doc_word in
                     ['readme', 'changelog', 'history', 'license', 'contributing', 'install', 'usage']):
        ...
        if parent_dir in ['.github', '.gitlab', 'ci', 'deploy', 'deployment'] or \
           'workflow' in filename or 'pipeline' in filename:
    ...
    def _get_content_sample(self, file_path: Path) -> Optional[str]:
        ...
        try:
            ...
            if file_path.suffix.lower() in ['.md', '.txt', '.rst', '.yaml', '.yml', '.json',
    ...
    def _create_generic_purpose_text(self, file_path: Path, primary_purpose: str,
    ...
    def _assess_generic_file_quality(self, file_justification: FileJustification) -> Dict[str, Any]:
        ...
        if filename in ['readme.md', 'license', 'changelog.md'] and \
           'root' not in str(file_justification.file_path).lower():
            issues.append("misplaced_project_file")
            quality_score -= 10

        # Check for configuration files without clear purpose
        if 'config' in file_justification.primary_purpose and \
           not file_justification.module_docstring:
        ...
        return {
            ...
            "complexity_assessment": "high" if file_justification.complexity_score > 8 else
                                    "medium" if file_justification.complexity_score > 4 else "low"
        }
                ...
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id == '__all__':
                        if isinstance(node.value, ast.List):
                            exports.extend([elt.s for elt in node.value.elts
    ...
    def _create_purpose_text(self, file_path: Path, primary_purpose: str,
                           secondary_purposes: List[str], docstring: Optional[str]) -> str:
    ...
    def _create_method_purpose_text(self, method_name: str, docstring: Optional[str],
                                  unique_functionality: str) -> str:
    ...
    def _assess_analysis_quality(self, file_justification: FileJustification,
                               method_justifications: List[MethodJustification]) -> Dict[str, Any]:
        ...
        return {
            ...
            "issues": issues,
            "documentation_ratio": documented_methods / len(method_justifications) if method_justifications else 0,
            "complexity_assessment": "high" if file_justification.complexity_score > 50 else "medium" if file_justification.complexity_score > 20 else "low"
        }
    ...
    def _find_embedding_redundancies(self) -> List[RedundancyCluster]:
        ...
        for method in self.method_justifications:
            if method.purpose_embedding:
                ...
                method_signatures.append(f"{method.file_path}:{method.line_number}:{method.method_name}")

        if len(method_embeddings) > 1:
            method_clusters = self._cluster_by_similarity(method_embeddings, method_signatures, "method")
            clusters.extend(method_clusters)

        return clusters

    def _cluster_by_similarity(self, embeddings: List[List[float]], identifiers: List[str],
                             cluster_type: str) -> List[RedundancyCluster]:
        ...
        for i in range(len(embeddings)):
            ...
            if similar_indices:
                ...
                common_purpose = self._extract_common_purpose_from_cluster(cluster_items, cluster_type)

                # Generate recommendation
                recommendation = self._generate_redundancy_recommendation(cluster_items, cluster_type, avg_similarity)
    ...
    def _extract_common_purpose_from_cluster(self, items: List[str], cluster_type: str) -> str:
        ...
        if cluster_type == "file":
            ...
            for item in items:
                ...
                for method in self.method_justifications:
                    if (method.file_path == file_path and
    ...
    def _generate_redundancy_recommendation(self, items: List[str], cluster_type: str,
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        files_with_unclear_purpose = sum(
            1 for f in self.file_justifications.values()
            if f.primary_purpose.startswith("unclear")
        )

        methods_without_docs = sum(
            1 for m in self.method_justifications
            if not m.docstring and m.complexity_score > 3
        )
        ...
        return {
            ...
            "justification_quality": {
                ...
                "documentation_coverage": (total_methods - methods_without_docs) / total_methods if total_methods > 0 else 0
            },
            "redundancy_analysis": {
    ...
    def _generate_recommendations(self, redundancies: List[RedundancyCluster]) -> List[Dict[str, Any]]:
        ...
        unclear_files = [f for f in self.file_justifications.values()
                        if f.primary_purpose.startswith("unclear")]
        if unclear_files:
            recommendations.append({
                "priority": "medium",
        ...
        complex_undocumented = [m for m in self.method_justifications
                               if not m.docstring and m.complexity_score > 5]
